...
function launcher(saved, storageFails = false) {
  const elements = Object.fromEntries(['server', 'setup', 'open', 'exit', 'error'].map(id => [id, { value: '', textContent: '', focus() { document.activeElement = this; } }]));
  ...
  vm.runInNewContext(source, { document, window, localStorage, URL });
  return { elements, window, storage, submit(address) { elements.server.value = address; elements.setup.onsubmit({ preventDefault() {} }); } };
}
